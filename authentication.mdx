...
description: "Obtain a JWT Bearer token from the login endpoint and pass it on every protected request using the Authorization header."
---
...
Used on routes that serve both anonymous and authenticated users differently. If no token is present, `req.user` is set to `null` and the request continues. If a valid token is present, `req.user` is populated exactly as with `Token`.

```javascript
// req.user will be null for unauthenticated requests:
router.post("/some-public-route", TokenOptional, handler);
```
...
| Invalid / malformed token | `false` (request continues) |
| Expired token | `403` returned immediately |
