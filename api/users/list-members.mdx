...
description: "Retrieve a paginated list of all registered users. Requires a valid authentication token. Page and per-page size are set via URL path parameters."
---

This endpoint returns all users stored in the database, sorted by creation date in descending order (newest first). Pagination is controlled through optional URL path parameters rather than query strings. The `Paginate` middleware computes the correct `skip` and `limit` values from those path segments before the controller runs.
...
<ResponseField name="data" type="array">
  Array of user documents for the requested page. Each document contains all fields stored on the `user` model (name, email, roles, status, address, phone_number, typeIdentification, identification, avatar, membership, codeseller, meseller, timestamps, etc.).
</ResponseField>
